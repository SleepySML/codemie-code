...
describe('BaseAgentAdapter', () => {
  ...
  // Characterisation tests for checkVersionCompatibility() were retired
  // together with the method itself when pinned per-agent supported-version
  // constants were removed. See BaseAgentAdapter.version-warning.test.ts for
  // the successor coverage of getVersionInfo() / warnOnceIfUntested().
});
