...
describe('BaseAgentAdapter', () => {
  ...
  // Characterisation tests locking the current shape of checkVersionCompatibility().
  // These pin down the input to the version-check branches inside run() before we
  // rewire that block in Task 5. They will be replaced when checkVersionCompatibility
  // itself is removed in Task 7.
  describe('checkVersionCompatibility (pre-refactor characterisation)', () => {
    beforeEach(() => vi.clearAllMocks());

    const baseMeta = (overrides: Partial<AgentMetadata>): AgentMetadata => ({
      name: 'test',
      displayName: 'Test',
      description: 'Test agent for characterisation',
      npmPackage: null,
      cliCommand: null,
      envMapping: {},
      supportedProviders: ['openai'],
